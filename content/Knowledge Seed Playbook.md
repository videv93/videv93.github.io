...
## Tám lần chạy tham chiếu

| | `2. Areas/UIUX/` | `2. Areas/Quant/` | `2. Areas/Physics/` | `2. Areas/Blockchain/` | `2. Areas/Frontend/` | `2. Areas/GIS/` | `2. Areas/SEO/` | `2. Areas/Marketing/` |
|---|---|---|---|---|---|---|---|---|
| Ngày chạy | 2026-08-27 | 2026-08-30 | 2026-09-01 | 2026-09-01 | 2026-09-02 | 2026-09-02 | 2026-09-02 | 2026-09-09 |
| Seed vào | 4 file / 292 dòng | 29 file / ~8.000 dòng | **3 file / 3.981 dòng** | **39 file / ~6.600 dòng** — *nằm ở area khác* | **67 file / 3.944 dòng** — *đã có sẵn `_archive-frontend/`* | **1 file / 215 dòng** — *toàn link khoá học* | **0 file / 0 dòng** — *không tồn tại* | **0 file / 0 dòng** — *không tồn tại*; xương sống là **một khoá học bên ngoài** |
| Loại seed | **Dàn ý** — 12 header rỗng | **Nguyên liệu thô** — transcript YouTube, lesson page, README | **Lai** — 2.400 dòng transcript + 5 header rỗng trong *cùng một file* | **Lai, phân tán** — 3 báo cáo audit + ~30 stub rỗng, rải trong `4. Archived/` | **Danh mục** — 4 index MDN + ~45 stub class list + 3 file MOC toàn wikilink gãy | **Danh mục — biến thể lộ trình**: ~90 link sắp theo thứ tự học, mỗi mục hứa cả một *môn*, không phải một *khái niệm* | **Không có seed** — greenfield thật; `grep` hai vòng toàn vault ra 0 kết quả | **Không có seed + một syllabus** — greenfield, nhưng có một dàn ý *do bên bán công cụ viết* làm điểm khởi đầu |
| Kết quả | 43 note / ~32.000 từ / 10 thư mục | 54 note / ~56.000 từ / 10 thư mục | 64 note / 10 thư mục | 68 note / ~63.000 từ / 10 thư mục | 64 note / ~75.000 từ / 10 thư mục | 66 note / ~68.000 từ / 10 thư mục | 67 note / ~71.000 từ / 10 thư mục | 75 note / ~63.000 từ / 10 thư mục |
| Nút thắt | Quyết định "khái niệm sống ở note nào" | **Đọc hết seed** + khử trùng lặp | Nhận ra nửa file là header rỗng | **Tìm ra seed** — folder đích rỗng hoàn toàn | **Đặt tên** — tên chung chung sẽ cướp wikilink của area khác | **Nhận ra cái seed *không hề nhắc tới*** — hệ toạ độ và viễn thám | **Chứng minh seed không tồn tại**, rồi dựng dàn ý lĩnh vực từ đầu | **Nhận ra syllabus phủ chưa tới một nửa lĩnh vực** — và phần thiếu là phần quan trọng nhất |
| Đặc thù | Header rỗng = danh sách việc | Xung đột hệ giá trị → cần **note bản lề** | Xung đột **nguồn ↔ chính nguồn đó** → note bản lề | Xung đột **ba bên trong cùng một tài liệu** → **hai** note bản lề | Xung đột **theo trục thời gian** (tài liệu bền vs tài liệu hết hạn) → **hai** note bản lề | Xung đột **theo trục thời gian, trong cùng một bảng** (nguyên lý và giá tiền cạnh nhau, cùng một giọng) → note bản lề | Xung đột **giữa nguồn và động cơ của nguồn** (Google nói ↔ dữ liệu ngành ↔ leak) + xung đột **tốc độ thay đổi** → **hai** note bản lề | Xung đột **ba trục cùng lúc**: hệ giá trị (brand↔performance), đo lường (attributed↔incremental), động cơ nguồn (vendor↔evidence) → **ba** note bản lề |
...
### 6.9 — Từ lần chạy Marketing (không có seed, nhưng có một syllabus của bên bán hàng)

- **⚠️ Bài học lớn nhất: một khoá học *không phải* một dàn ý lĩnh vực.** 6.8 dạy rằng khi không có seed, phép kiểm "dàn ý lĩnh vực độc lập" trở thành toàn bộ Bước 1. Lần này người dùng đưa sẵn **một** dàn ý — syllabus Adobe Marketing Specialist. Cám dỗ là coi nó như seed và chỉ việc mở rộng 5 khoá thành note. Đối chiếu với ba dàn ý độc lập (Kotler *Marketing Management* 16e, tài liệu đo lường MMM/incrementality 2026, chương trình CIM) cho thấy khoá này **không nhắc một lần nào** tới: STP, pricing, nghiên cứu thị trường, chiến lược thương hiệu, kinh tế đơn vị, quyền riêng tư, và **toàn bộ tầng đo lường nhân quả**. Đó là ~50% khối lượng vault và là nửa quan trọng hơn.
  > **Quy tắc mới:** khi seed là một **syllabus/khoá học**, luôn hỏi *"ai sản xuất khoá này và họ bán gì?"* trước khi dùng nó làm cây thư mục. Adobe dạy marketing bằng công cụ Adobe, nên trọng tâm rơi vào sản xuất nội dung — hoàn toàn hợp lý cho họ, và hoàn toàn không đủ làm dàn ý lĩnh vực. Ánh xạ khoá vào vault thành một note riêng ghi rõ **cả phần nó dạy lẫn phần nó không dạy**; ở đây là `Adobe Marketing Specialist Course Map`, và mục "khoá này không dạy gì" là mục giá trị nhất của note đó.
- **Loại xung đột thứ sáu, và lần đầu cần *ba* note bản lề.** Các lần trước: hai nguồn (Quant), hai phần một nguồn (Physics), ba vai trò (Blockchain), hai chu kỳ bán rã (Frontend/GIS), động cơ nguồn (SEO). Marketing có **ba trục độc lập cùng lúc**:
  | Note bản lề | Trục | Câu hỏi nó trả lời |
  |---|---|---|
  | `Brand Building vs Performance Marketing` | Hệ giá trị | Cái gì được tính là tăng trưởng? |
  | `Attributed vs Incremental` | Đo lường | Con số này có nghĩa là gì? |
  | `Vendor Playbook vs Marketing Evidence` | Động cơ nguồn | Ai đang nói với tôi, và họ bán gì? |
  Ba tập note liên quan **giao nhau nhiều** — trái với dấu hiệu tách ở 6.5. Nhưng 6.8 đã bổ sung dấu hiệu thứ hai (*trục khác nhau thì tách*), và lần này xác nhận nó mạnh hơn dấu hiệu tập note: một tuyên bố có thể **đúng về nhân quả** mà vẫn **đến từ nguồn có động cơ**, và vẫn **đo sai chân thời gian**. Ba câu hỏi trực giao → ba note.
- **Thiên lệch của nguồn có thể *một chiều và có hệ thống*, không chỉ "cần trừ hao".** Ở SEO, xung đột động cơ làm nguồn *kém tin cậy*. Ở Marketing, ba cơ chế độc lập (chọn mẫu tự chọn, nhiều nền tảng cùng nhận công, kênh không click được thì vô hình) đều đẩy sai số về **cùng một hướng**. Hệ quả viết note: không được khuyên "hãy thận trọng với số này" — phải nói rõ **hướng** của sai lệch, vì nó quyết định hành động ngược lại hoàn toàn với trực giác (kênh trông tệ nhất trong dashboard thường là kênh bị đo sai nhất).
- **Phép kiểm va chạm tên: 75/75 sạch ngay lần đầu — lần thứ hai liên tiếp.** Áp quy ước tiền tố **trước khi soạn danh sách** (`Marketing Fundamentals`, `Marketing Research Methods`, `Brand Positioning Statement`…) biến phép kiểm thành xác nhận. Nhưng ba tên vẫn phải đổi sau khi grep khái niệm: UIUX đã sở hữu `Persona & JTBD` và `User Journey & Flow`, nên `Jobs To Be Done` → `JTBD for Market Demand`, `Customer Persona Craft` → `Marketing Personas and ICP`, `Customer Journey Mapping` → `Buyer Journey and Touchpoints`. **Bài học: phép kiểm va chạm *tên file* không bắt được va chạm *khái niệm*.** Phải chạy thêm một vòng `find` theo tên khái niệm ở các area lân cận — ở đây SEO và UIUX chồng lấn nặng với Marketing (12 khái niệm có nhà ở area khác).
- **Vault đông thì mục "Khái niệm có nhà ở area khác" phải là *bảng ba cột*.** 6.6 đề xuất mục này, 6.7 chuyển nó vào Bước 3. Lần này nó cần thêm cột thứ ba — *Marketing dùng nó ở đâu* — vì với 12 khái niệm chồng lấn, biết "SEO sở hữu Keyword Research" chưa đủ; phải biết note nào của mình được phép link sang. Không có cột đó thì 6 tháng sau vẫn viết trùng.
- **`status` frontmatter tiếp tục là chỉ báo độ tươi dùng được.** 40 `evergreen` / 31 `growing` / 3 `seed`, và phân bố không ngẫu nhiên: `seed` rơi đúng vào ba note có hạn sử dụng ngắn nhất (`Generative AI in Marketing Creative`, `Social Platform Mechanics`, `AI Disclosure and Content Provenance`); `growing` rơi vào các note gắn với công cụ và nền tảng cụ thể. MOC chỉ thẳng vào quy ước này ở mục "Cách dùng vault". Xác nhận bài học 6.8, lần thứ hai.
- **Nghĩa địa chiến thuật lại xuất hiện, và lần này nó lớn hơn SEO.** `Marketing Tactics Graveyard` có 15 chiến thuật đã chết vẫn được dạy — trong đó **bốn** cái vẫn là *mặc định* ở phần lớn tổ chức (last-click, open rate làm KPI, nhắm siêu hẹp trên paid social, gate mọi nội dung). Bổ sung cho 6.8: bảng nghĩa địa nên có thêm mục **"vì sao nó vẫn sống"** — ở đây bốn cơ chế (nội dung cũ xếp hạng cao, khoá học cập nhật chậm, công cụ vẫn hiển thị chỉ số cũ, và **chiến thuật chết vẫn cho số liệu đẹp**). Cơ chế thứ tư là cơ chế nguy hiểm nhất và chỉ lộ ra khi đã có note bản lề về đo lường.
- **Viết MOC trước lại cho 0 link gãy — lần thứ sáu liên tiếp.** 934 wikilink, 75 đích nội bộ, 0 gãy ngay lần đầu. 25 mục hiện ra trong phép kiểm đều là note ngoài folder và **cả 25 đều tồn tại thật** (đã `find` từng cái, đúng như cảnh báo ở Bước 6). Không lần nào phải sửa wikilink viết theo quán tính — khác với GIS (`[[GeoParquet]]`) và SEO (`[[Google Trends]]`, `[[Indexing API]]`). Khác biệt có thể do lần này danh sách công cụ được gom vào **một** note catalogue thay vì rải trong các note khái niệm, nên không có chỗ để quán tính "liệt kê công cụ → viết wikilink" phát sinh.
- **Ước lượng quy mô: đếm số *môn* seed hứa vẫn đúng, nhưng phải cộng cả môn seed *không* hứa.** Syllabus hứa ~5 môn (design, AI, digital marketing, social, multichannel) → nếu chỉ theo nó thì ra ~35–40 note. Cộng phần bù từ ba dàn ý độc lập (STP, pricing, research, brand, đo lường, kinh tế đơn vị, pháp lý) → 75 note. **Tỉ lệ giữa "phần seed hứa" và "phần seed thiếu" ở đây là gần 50/50** — cao nhất trong tám lần chạy, và là lý do bước đối chiếu dàn ý độc lập của 6.7 không được bỏ qua kể cả khi đã có sẵn một syllabus trông có vẻ đầy đủ.
...
## Liên kết
[[UIUX]] · [[Quant]] · [[Physics]] · [[Blockchain]] · [[Frontend]] · [[GIS]] · [[SEO]] · [[Marketing]] — tám lần chạy tham chiếu, xem kết quả đầy đủ ở đó.
